...
params:
    province: "Alberta"
bibliography: hist_horse_pop_refs.bib
---
...
```{r plot horses, fig.width=10, fig.height=3, fig.cap = "Plot 1. Historical number of horses per province in Canada"}
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable(max_horses, caption = "Table 1. range of the number horses for each provinces at any time point between 1940 - 1972") |> kableExtra::kable_styling(full_width = FALSE)
```

Below we zoom in and look at the province of `r params$province`:

```{r plot province, fig.width=5, fig.height=4, fig.cap = "Plot 2. Historical number of horses for the province of interest"}
province_plot <- horse_pop %>% 
  ...
  xlab("Year") +
  ylab("Number of horses") +
  ggtitle(paste("Historical number of horses for", params$province)) +
  facet_grid(~GEO) +
  scale_y_continuous(labels = scales::comma) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))
province_plot
